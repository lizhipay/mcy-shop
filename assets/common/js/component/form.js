class Form {
        ...
        for (let i = 0; i < this.opt.tab.length; i++) {
            ...
            for (let j = 0; j < item.form.length; j++) {
                ...
                if (form.required === true && value === "") {
                    layer.msg(`「${form.title ?? form.name}」${i18n('不能为空值')}`);
                    return false;
                }
                if (form.regex && value) {
    ...
    getBlockHtml(form, widgetHtml = "") {
        return `<div class="layui-form-item block-${form.name} ${this.data[form.name]['hide']}">
            <label class="layui-form-label ${this.data[form.name].titleHide}">${form.title}${form.required === true ? util.icon('icon-bitian') : ''}</label>
    ...
    editorHtml(form) {
        return this.getBlockHtml(form, `<div class="editor-wrapper"><div><button data-type="0" class="button-switch-${form.name}" type="button" style="width: 100%;border: none;background: white;border-radius: 5px 5px 0 0;color: #c9b8b8;"><i class="fas fa-code" style="color: #c9b8b8;"></i> HTML</button></div><div class="editor-content"><div class="toolbar-container"></div><div class="editor-container"></div></div><textarea class="text-container" style="display: none;" name="${form.name}">${form.default ?? ""}</textarea></div>`);
    }
    ...
    imageHtml(form) {
        if (form.photoAlbumUrl) {
            form.title = form.title ? form.title += `<a class="photo-album" style="position: relative;top: 2px;cursor:pointer;">${util.icon('icon-meijingxiangce', "icon-16px")}</a>` : form?.title;
        }


        form.title += `<a class="external-input" style="position: relative;top: 2px;cursor:pointer;">${util.icon('icon-wailianshezhi', "icon-14px")}</a>`;
    ...
    addWidget(name, instance = null, val = {}) {
        ...
        let html = '' +
            ...
            '</select></div> ' +
            '<input type="text"  name="title-' + name + '[]" placeholder="' + i18n("控件名称") + '" class="layui-input widget-general widget-w120" value="' + (val.hasOwnProperty("title") ? val.title : "") + '"> ' +
            ...
            '<textarea  name="data-' + name + '[]" type="text" placeholder="' + i18n("请提供配置可选择的多个数据，例子：&#10;大熊猫=dxm&#10;小熊猫=xxm&#10;每行一个数据，多个数据使用多行即可，格式：[显示名称]=[数据内容]") + '" class="layui-textarea widget-data widget-data-' + unique + '">' + (val.data ?? "") + '</textarea> ' +
            '</div>';
    ...
    uploadImage(opt = {}) {
        ...
        if (opt.imageUrl) {
            ...
            imageContainer.html('<button type="button" class="layui-btn btn-upload image-upload"><i class="layui-icon layui-icon-picture"></i><space></space>' + opt.title + '</button>');
        }
        layUpload.render({
                ...
                switch (form.type) {
                    ...
                    case 'widget':
                        ...
                        obj["name-" + form.name].forEach((name, index) => {
                            if (name != "") {
                                json.push({
                                    title: obj["title-" + form.name][index],
                                    name: name,
    ...
    editorRegister(form) {
        ...
        $('.' + _this.unique + ' .component-' + form.name + ' .button-switch-' + form.name).click(function () {
            ...
            if (type == 0) {
                ...
                _obj.html('<i class="fas fa-feather" style="color: #c9b8b8;"></i><space></space>' + i18n("写作"));
                ...
                _obj.html('<i class="fas fa-code" style="color: #c9b8b8;"></i> ' + "HTML");
    ...
    imageRegister(form) {
        ...
        $externalInput.click(() => {
            ...
            component.popup({
                ...
                tab: [
                    {
                        name: util.icon('icon-wailianshezhi') + " 设置外部图片链接",
                        form: [
                            {
                                title: false,
                                name: "url",
        ...
        if (form.photoAlbumUrl) {
            ...
            $photoAlbum.click(function () {
                ...
                component.popup({
                    ...
                    tab: [
                        {
                            name: util.icon("icon-meijingxiangce") + " 相册",
                            form: [
                                {
                                    name: "photo_album",
                                    type: "custom",
